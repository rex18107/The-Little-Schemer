...
; 将元素叠加到空列表上
(define (all-item i1 i2 i3)
  (cons i1
        (cons i2
              (cons i3 '()))))
; 根据参数num的三个值（0 1 2），返回对应的数值
(define (subst-group num)
  (cond
    ((eq? num 0) 0)
    ((eq? num 1) 1)
    (else
...
; 使用col函数作为分类器，将lat列表分为三组子列表元素
(define (group-a lat test? col)
  (cond
    ; 终止条件,将两个空列表作为参数传入进col中
    ((null? lat)
     (col '() '() '()))
    ; 将test?返回为0的元素,放入col的第一个参数
    ((eq? (test? (car lat)) 0)
     (group-a (cdr lat) test? (lambda (nl l1 l2)
                              ; 这里是将新的参数(cons (car lat) nl)和l1，l2
                              ; 传入到上个递归里col的函数内容中
                              (col (cons (car lat) nl)
                                   l1 l2))))
     ; 将test?返回为1的元素,放入col的第二个参数
    ((eq? (test? (car lat)) 1)
     (group-a (cdr lat) test? (lambda (nl l1 l2)
                                (col nl
                                   (cons (car lat)l1)
                                   l2))))
    (else
     ; 将test?返回为2的元素,放入col的第三个参数
     (group-a (cdr lat) test? (lambda (nl l1 l2)
                              (col nl
                                   l1
                                   (cons (car lat) l2)))))))
; ((0 0 0) (1 1 1) (2 2 2 2))
(group-a '(1 0 2 1 0 2 2 1 2 0)  subst-group all-item)
